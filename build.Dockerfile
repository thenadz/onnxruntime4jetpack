...
RUN --mount=type=cache,target=/ort-src \
    if [ -d /ort-src/.git ]; then \
        echo "ORT source cache exists — updating"; \
        git -C /ort-src fetch --shallow-exclude=${ORT_BASE_COMMIT} origin ${ORT_VERSION} && \
        git -C /ort-src reset --hard FETCH_HEAD && \
        git -C /ort-src submodule update --init --recursive; \
    else \
        echo "ORT source cache empty — cloning"; \
        git clone --single-branch --branch ${ORT_VERSION} \
            --shallow-exclude=${ORT_BASE_COMMIT} --recursive \
